...
describe('record.token', () => {
  ...
  it('should properly parse available field', () => {
    expect(tokenRecord.available).to
      .equal(recordResponses.token.available_for_issuance)
  })
  ...
  it('should properly parse signer field', () => {
    expect(tokenRecord.signer).to
      .equal(recordResponses.token.preissued_asset_signer)
  })
  it('should properly parse max field', () => {
    expect(tokenRecord.max).to
      .equal(recordResponses.token.max_issuance_amount)
  })
  ...
  it('getter requiresKYC should return false ' +
    'to asset which doesn\'t require KYC', () => {
    expect(tokenRecord.requiresKYC).to.equal(false)
  })
  it('getter requiresKYC should return true ' +
    'to asset which requires KYC', () => {
  ...
  it('getter logoURL should return correct url to token with logo URL', () => {
    expect(hasLogoTokenRecord.logoURL).to.equal(
      `${config.FILE_STORAGE}/${recordResponses.tokenWithLogo.details.logo.key}`
    )
  })
  it('getter logoURL should return empty string ' +
    'to token without logo URL', () => {
    expect(tokenRecord.logoURL).to.equal('')
  })
  it('should attach details provided in constructor', () => {
    const tokenRecordWithDetails = new TokenRecord(recordResponses.token,
      details)
