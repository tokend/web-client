...
describe('AssetCreateRequestRecord', () => {
  ...
  const setLogoKey = (rawJSON, key) => {
    rawJSON.details.create_asset.creator_details.logo = {}
    rawJSON.details.create_asset.creator_details.logo.key = key

    return rawJSON
  }

  const setTermsKey = (rawJSON, key) => {
    rawJSON.details.create_asset.creator_details.terms = {}
    rawJSON.details.create_asset.creator_details.terms.key = key
  ...
  it('constructor properly parsed all the basic fields', () => {
    ...
    expect(record.assetName)
      .to
      .equal(
        assetCreateRecordJSON.details.create_asset.creator_details.name
      )
    ...
    expect(record.externalSystemType)
      ...
      .deep
      .equal(
        assetCreateRecordJSON.details.create_asset.creator_details
  ...
  it('constructor properly parsed the logo details', () => {
    ...
    expect(record.logo)
      ...
      .deep
      .equal(
        assetCreateRecordJSON.details.create_asset.creator_details.logo
      )

    expect(record.logoKey)
      .to
      .equal(
        assetCreateRecordJSON.details.create_asset.creator_details.logo.key
      )

    expect(record.logoName)
      .to
      .equal(
        assetCreateRecordJSON.details.create_asset.creator_details.logo.name
      )

    expect(record.logoType)
      .to
      .equal(
        assetCreateRecordJSON.details.create_asset.creator_details.logo.type
      )
  })
  ...
  it('constructor properly parsed terms details', () => {
    ...
    expect(record.terms)
      ...
      .deep
      .equal(
        assetCreateRecordJSON.details.create_asset.creator_details.terms
      )

    expect(record.termsKey)
      .to
      .equal(
        assetCreateRecordJSON.details.create_asset.creator_details.terms.key
      )

    expect(record.termsName)
      .to
      .equal(
        assetCreateRecordJSON.details.create_asset.creator_details.terms.name
      )

    expect(record.termsType)
      .to
      .equal(
        assetCreateRecordJSON.details.create_asset.creator_details.terms.type
      )
  })
