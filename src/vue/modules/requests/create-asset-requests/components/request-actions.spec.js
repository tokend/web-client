...
describe('Create asset request actions', () => {
  let sandbox
  let wrapper
  ...
  describe('computed property', () => {
    describe('canBeUpdated', () => {
      it('returns true only for rejected and pending requests', () => {
        wrapper.setProps({
          request: new CreateAssetRequest({ stateI: REQUEST_STATES.rejected }),
        })
        expect(wrapper.vm.canBeUpdated).to.be.true

        wrapper.setProps({
          request: new CreateAssetRequest({ stateI: REQUEST_STATES.pending }),
        })
        expect(wrapper.vm.canBeUpdated).to.be.true

        wrapper.setProps({
          request: new CreateAssetRequest({ stateI: REQUEST_STATES.approved }),
        })
        expect(wrapper.vm.canBeUpdated).to.be.false

        wrapper.setProps({
          request: new CreateAssetRequest({ stateI: REQUEST_STATES.canceled }),
        })
        expect(wrapper.vm.canBeUpdated).to.be.false
  ...
  describe('methods', () => {
    describe('cancelRequest', () => {
      it('calls proper set of methods and emits proper event if request cancellation succeded', async () => {
        ...
        sandbox.stub(wrapper.vm, 'cancelCreateAssetRequest').resolves()
      ...
      it('calls ErrorHandler.process and sets isRequestCanceling to false if an error was thrown', async () => {
        sandbox.stub(wrapper.vm, 'cancelCreateAssetRequest').rejects()
