<template>
  ...
  >
    ...
    <no-data-message
      ...
      :title="'create-atomic-swap-form.no-owned-assets-title' | globalize"
      :message="'create-atomic-swap-form.no-owned-assets-msg' | globalize"
    />
  </form>
</template>
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      quoteAssets: vuexTypes.fiatAssets,
  ...
  async created () {
    ...
    this.form.quoteAssets[0].asset = this.quoteAssets[0] || {}
  },
  methods: {
    setAssetByCode (code) {
    ...
    setQuoteAssetByCode (code, index) {
      this.form.quoteAssets[index].asset = this.quoteAssets
        .find(item => item.code === code)
    },
    ...
    buildCreateAtomicSwapOperation () {
      ...
      this.form.quoteAssets.forEach(quoteAsset => {
        addresses['btc'] = quoteAsset.address
        quoteAssets.push({
