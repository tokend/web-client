    ...
    <template v-if="isLoaded">
      <div class="dashboard__toolbar">
        <asset-selector
          ...
          :current-asset="currentAsset.code"
          @asset-change="currentAsset.code = $event"
          :scale="scale"
        />
          ...
          >
            ...
            {{
              'dashboard.send-asset-lbl' | globalize({
                asset: currentAsset.code })
            }}
          </button>
        </div>
      </div>
      <template v-if="currentAsset.code">
        <div
          v-if="currentAsset.code !== defaultQuoteAsset"
          class="dashboard__chart"
        >
          <chart
            :base-asset="currentAsset.code"
            :quote-asset="defaultQuoteAsset"
          />
        </div>
        <div
          class="dashboard__activity"
          v-if="currentAsset.code"
        >
          <movements-history-module
            :asset-code="currentAsset.code"
      ...
      <drawer :is-shown.sync="showDrawer">
        ...
        <template v-if="transferFormIsShown">
          ...
          <transfer
            ...
            :asset-to-transfer="currentAsset.code"
          />
        </template>
      </drawer>
    </template>
  ...
  methods: {
    ...
    setCurrentAsset (value) {
      if (value) {
        ...
        this.currentAsset =
          keys.find(a => a.code === this.$route.query.asset) || keys[0] || ''
      }
    },
