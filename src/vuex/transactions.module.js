...
export const mutations = {
  [vuexTypes.UPDATE_TX_LIST] (state, tokenCodes) {
  ...
  [vuexTypes.UPDATE_TX_LIST_ITEM] (state, { code, paginator }) {
    const lists = cloneDeep(state.lists)
    lists[code] = paginator
    state.lists = lists
  },

  [vuexTypes.SET_TX_LIST_INITIALIZED] (state) {
    state.isInitialized = true
  }
}

export const actions = {
  [vuexTypes.INIT_TX_LIST] ({ commit, rootGetters }) {
  ...
  async [vuexTypes.GET_TX_LIST] ({ state, dispatch, commit }, tokenCode) {
  ...
  async [vuexTypes.NEXT_TX_LIST] ({ state, commit, dispatch }, tokenCode) {
    const paginator = state.lists[tokenCode]
    await paginator.next()
    commit(vuexTypes.UPDATE_TX_LIST_ITEM, paginator)
  },

  async [vuexTypes.UPDATE_TX_COUNTERPARTIES] ({ commit }, transactions) {
...
export const getters = {
  [vuexTypes.transactions]: state => state.lists
}
